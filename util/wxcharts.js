...
function findRange(num, type, limit) {
  if (isNaN(num)) {
    throw new Error("[ttxCharts] unvalid series data!");
  }
  limit = limit || 10;
...
function drawPieText(series, opts, config, context, radius, center) {
  ...
  textObjectCollection.forEach(function (item) {
    var lineStartPoistion = convertCoordinateOrigin(
      item.lineStart.x,
      item.lineStart.y,
      center
    );
    var lineEndPoistion = convertCoordinateOrigin(
      item.lineEnd.x,
      item.lineEnd.y,
      center
    );
    var textPosition = convertCoordinateOrigin(
      item.start.x,
      item.start.y,
      center
    );
...
var Charts = function Charts(opts) {
  ...
  this.context = tt.createCanvasContext(opts.canvasId);
